...
void ray(float ang, RAY *r) {
  ...
  const SEGMENT lineB = { { player.pos.x, player.pos.y },
                        { (cos(ang * 180 / M_PI) * 10 + player.pos.x), (sin(ang * 180 / M_PI) * 10 + player.pos.y) },
                        NULL };
...
void min_distance( int NOR ) {
  for (int i = 0; i < NOR; i++) {
    ...
    for (int j = 1; j < world.segment_c; j++) {
      if ( !(rays[i].distances[j] < 0 || rays[i].distances[j - 1] < 0)
